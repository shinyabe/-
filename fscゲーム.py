...
# === 生産者からの1日あたり供給可能量の範囲（ランダム供給用） ===
# 例として [50, 300] 個の範囲から一様ランダムに供給能力を決定（必要に応じて調整）
SUPPLY_MIN = {"L": 400, "S": 400}
SUPPLY_MAX = {"L": 900, "S": 900}

# 生産者から卸への1日あたり供給量の分布パラメータ（正規分布用）
# 平均は範囲の中央、標準偏差は「だいたい ±3σ が [MIN, MAX] に収まる」イメージで設定
SUPPLY_MU = {
    "L": (SUPPLY_MIN["L"] + SUPPLY_MAX["L"]) / 2.0,
    "S": (SUPPLY_MIN["S"] + SUPPLY_MAX["S"]) / 2.0,
}
SUPPLY_SIGMA = {
    "L": (SUPPLY_MAX["L"] - SUPPLY_MIN["L"]) / 6.0,
    "S": (SUPPLY_MAX["S"] - SUPPLY_MIN["S"]) / 6.0,
}
...
"""
"""
PENALTY_LOST_W  = 120.0  # 卸の売り逃しペナルティ（客一人分の利益イメージ）
PENALTY_LOST_R  = 120.0  # 小売の売り逃しペナルティ（客一人分の利益イメージ）
"""

PENALTY_LOST_W  = 60.0  # 卸の売り逃しペナルティ（客一人分の利益イメージ）120
PENALTY_LOST_R  = 120.0  # 小売の売り逃しペナルティ（客一人分の利益イメージ）120
...
def plot_customers_and_demands(T: int = 30):
    ...
    for g in ACTIVE_G:              # 各カテゴリごとにグラフを描く
        ...
        plt.plot(days, D_list, label=f"{name}: 需要量 D")  # 需要量の折れ線グラフ
        plt.xlabel("日数 t")                        # x軸ラベル
        plt.ylabel("人数 / 個数")                  # y軸ラベル
        plt.title(f"{name}の客入り・需要")  # グラフタイトル
        plt.legend()                                # 凡例を表示
...
def set_transitions_for_node(n: str):  # 節点 n（W=卸, R=小売）ごとに、カテゴリ・鮮度状態間の遷移確率を設定する関数
    # P0_base: 通常保存、P1_base: 高品質保存
    if n == "W":  # 卸の遷移確率（倉庫：比較的環境が安定している想定）
        # =======================
        # 根菜 L（もともと日持ち良い）
        # =======================
        # 根菜L, 鮮度1→1,2,3 の確率（通常保存）
        P0_base[(n, "L", 1, 1)] = 0.88  # かなりの確率で鮮度1のまま
        P0_base[(n, "L", 1, 2)] = 0.12
        P0_base[(n, "L", 1, 3)] = 0.00

        # 根菜L, 鮮度2→1,2,3 の確率（通常保存）
        P0_base[(n, "L", 2, 1)] = 0.00
        P0_base[(n, "L", 2, 2)] = 0.72  # 鮮度2にとどまる確率も高め
        P0_base[(n, "L", 2, 3)] = 0.28

        # 根菜L, 鮮度1→1,2,3 の確率（高品質保存）
        P1_base[(n, "L", 1, 1)] = 0.95  # 高品質保存でさらに劣化しにくく
        P1_base[(n, "L", 1, 2)] = 0.05
        P1_base[(n, "L", 1, 3)] = 0.00

        # 根菜L, 鮮度2→1,2,3 の確率（高品質保存）
        P1_base[(n, "L", 2, 1)] = 0.00
        P1_base[(n, "L", 2, 2)] = 0.85  # 鮮度2から3への落ちがかなり抑えられる
        P1_base[(n, "L", 2, 3)] = 0.15

        # =======================
        # 葉物 S（もともと日持ち悪い）
        # =======================
        # 葉物S, 鮮度1→1,2,3 の確率（通常保存）
        P0_base[(n, "S", 1, 1)] = 0.60  # 鮮度1に留まる確率は根菜よりかなり低め
        P0_base[(n, "S", 1, 2)] = 0.40
        P0_base[(n, "S", 1, 3)] = 0.00

        # 葉物S, 鮮度2→1,2,3 の確率（通常保存）
        P0_base[(n, "S", 2, 1)] = 0.00
        P0_base[(n, "S", 2, 2)] = 0.40
        P0_base[(n, "S", 2, 3)] = 0.60  # 一気に廃棄状態に落ちやすい

        # 葉物S, 鮮度1→1,2,3 の確率（高品質保存）
        P1_base[(n, "S", 1, 1)] = 0.80  # 高品質保存により、かなりマシにはなるが…
        P1_base[(n, "S", 1, 2)] = 0.20
        P1_base[(n, "S", 1, 3)] = 0.00

        # 葉物S, 鮮度2→1,2,3 の確率（高品質保存）
        P1_base[(n, "S", 2, 1)] = 0.00
        P1_base[(n, "S", 2, 2)] = 0.65  # 通常保存よりはだいぶマシ
        P1_base[(n, "S", 2, 3)] = 0.35

    else:  # 小売の遷移確率（店頭：照明・開閉などで卸よりやや劣化しやすい想定）
        # =======================
        # 根菜 L（店頭では卸より少し劣化しやすい）
        # =======================
        # 根菜L, 鮮度1→1,2,3（通常保存）
        P0_base[(n, "L", 1, 1)] = 0.80
        P0_base[(n, "L", 1, 2)] = 0.20
        P0_base[(n, "L", 1, 3)] = 0.00

        # 根菜L, 鮮度2→1,2,3（通常保存）
        P0_base[(n, "L", 2, 1)] = 0.00
        P0_base[(n, "L", 2, 2)] = 0.60
        P0_base[(n, "L", 2, 3)] = 0.40

        # 根菜L, 鮮度1→1,2,3（高品質保存）
        P1_base[(n, "L", 1, 1)] = 0.92
        P1_base[(n, "L", 1, 2)] = 0.08
        P1_base[(n, "L", 1, 3)] = 0.00

        # 根菜L, 鮮度2→1,2,3（高品質保存）
        P1_base[(n, "L", 2, 1)] = 0.00
        P1_base[(n, "L", 2, 2)] = 0.78
        P1_base[(n, "L", 2, 3)] = 0.22

        # =======================
        # 葉物 S（店頭ではかなり劣化しやすい）
        # =======================
        # 葉物S, 鮮度1→1,2,3（通常保存）
        P0_base[(n, "S", 1, 1)] = 0.45
        P0_base[(n, "S", 1, 2)] = 0.55
        P0_base[(n, "S", 1, 3)] = 0.00

        # 葉物S, 鮮度2→1,2,3（通常保存）
        P0_base[(n, "S", 2, 1)] = 0.00
        P0_base[(n, "S", 2, 2)] = 0.25
        P0_base[(n, "S", 2, 3)] = 0.75  # 店頭通常保存だとかなりすぐ廃棄へ

        # 葉物S, 鮮度1→1,2,3（高品質保存）
        P1_base[(n, "S", 1, 1)] = 0.75
        P1_base[(n, "S", 1, 2)] = 0.25
        P1_base[(n, "S", 1, 3)] = 0.00

        # 葉物S, 鮮度2→1,2,3（高品質保存）
        P1_base[(n, "S", 2, 1)] = 0.00
        P1_base[(n, "S", 2, 2)] = 0.55
        P1_base[(n, "S", 2, 3)] = 0.45
...
class SimStats: # シミュレーション統計情報を格納するクラス
    ...
    lost_W: float        # 卸側の売り逃し数
    lost_R: float        # 小売側の売り逃し数

class Simulator: # シミュレータ本体クラス
    # 輸送中の鮮度劣化確率（カテゴリごと）: (1→2 の確率, 2→3 の確率)
    SHIP_DECAY = {
        "L": (0.01, 0.02),  # 根菜：1→2 が 2%、2→3 が 0.5%
        "S": (0.1, 0.2),   # 葉物：1→2 が 10%、2→3 が 6%
    } # 輸送中の鮮度劣化確率（カテゴリごと）: (1→2 の確率, 2→3 の確率)
    ...
    def run(self) -> SimStats:
        ...
        for t in range(1, T + 1):  # 各日 t についてシミュレーション
            ...
            for g in ACTIVE_G: # 各カテゴリ g について
                ...
                desired = int(math.ceil(gap))             # 本来仕入れたい量
                # 生産者のその日の供給能力を「正規分布 + クリップ」で決定
                raw_supply = random.gauss(SUPPLY_MU[g], SUPPLY_SIGMA[g])  # N(μ, σ^2) からサンプリング
                # 負の値や極端な値を避けるため、[SUPPLY_MIN, SUPPLY_MAX] の範囲にクリップ
                raw_supply = max(SUPPLY_MIN[g], min(SUPPLY_MAX[g], raw_supply))
                supply_cap = int(round(raw_supply))  # 実際の供給能力を整数に丸める

                # 実際に仕入れられる量は「欲しい量」と「供給能力」の小さい方
                x_buy[g] = min(desired, supply_cap)
            ...
            lost_W_today = 0.0                           # その日の卸の売り逃し数量

            for g in ACTIVE_G: # 各カテゴリ g について
                need = max(0.0, target_R[g] - (self.I_R[g][1] + self.I_R[g][2]))  # 小売の不足量（鮮度1+2のみ）

                # 卸の在庫不足による売り逃し（小売の要求を満たせない分）
                available_W = self.I_W[g][1] + self.I_W[g][2]
                lost_W_g = max(0.0, need - available_W)
                lost_W_today += lost_W_g
            ...
            self.lost_W += lost_W_today           # 卸の売り逃し累計
...
def get_safety_candidates_R() -> Dict[str, List[int]]:
    ...
    inv_L = [400, 450, 500, 550, 600, 650, 700, 750, 800, 850, 900, 950, 1000]
    inv_S = [400, 450, 500, 550, 600, 650, 700, 750, 800, 850, 900, 950, 1000]
    """
...
def get_safety_candidates_W() -> Dict[str, List[int]]:
    ...
    base_L = [600, 650, 700, 750, 800, 850, 900, 950, 1000, 1050, 1100, 1150, 1200]
    base_S = [600, 650, 700, 750, 800, 850, 900, 950, 1000, 1050, 1100, 1150, 1200]
...
def payoff_W(res: dict) -> float:
    ...
    """
    return (res["profit_W"]
            - PENALTY_CO2_W * res["co2_W"]
            - PENALTY_WASTE_W * res["waste_W"]
            - PENALTY_LOST_W * res["lost_W"])
...
def best_response_R(base: DiscretePolicy, samples: int = 40) -> Tuple[DiscretePolicy, dict, float]:
    ...
    for _ in range(samples):
        cand = clone_with_R(
            ...
            discount=DiscountPolicy(use_discount=random.choice([True, False])),
            storage_R=StorageMode(mode=random.choice(["high", "low"])),
            ship_order_R=ShipOrder(order=random.choice(["LIFO", "FIFO"])),
        )
        choices.append(cand)
...
def show_result(title: str, res: dict):
    ...
    print(f" 卸利得(uW): {uW:.2f} / 小売利得(uR): {uR:.2f}  （利益 − ペナルティ）")
    ...
    print(f" 売り逃し: 卸 lost_W = {res['lost_W']:.2f} 個, 小売 lost_R = {res['lost_R']:.2f} 個")
    print("-" * 60)
    if "L" in ACTIVE_G:
...
def plot_strategy_profit_heatmaps(base_policy: DiscretePolicy,
    ...
    for i, w_pol in enumerate(W_list):          # 卸戦略インデックス i
        for j, r_pol in enumerate(R_list):      # 小売戦略インデックス j
            ...
            # ★ 追加：利得（ペナルティ込みの効用）を計算 ★
            uW = payoff_W(res)                  # 卸利得
            uR = payoff_R(res)                  # 小売利得

            # ★ここでヒートマップ用には「利益」ではなく利得を使用するよう変更★
            profit_W_mat[i, j] = payoff_W(res)  # 卸利得を行列に格納
            profit_R_mat[i, j] = payoff_R(res)  # 小売利得を行列に格納
            ...
            print(f"  卸利得(uW): {uW:.2f} 円")
            print(f"  小売利得(uR): {uR:.2f} 円")
            ...
            record = {
                ...
                "目利き戦略有無": "True" if w_pol.mekiki.level == "high" else "False",
            }
    ...
    plt.imshow(profit_W_mat, origin="lower", aspect="auto")  # 卸利得行列をヒートマップ表示
    plt.colorbar(label="卸の利得 [円]")                     # カラーバーにラベル（利得）
    plt.xticks(range(nR), x_labels, rotation=90, fontsize=5)             # x軸に小売戦略名を縦書きで表示
    plt.yticks(range(nW), y_labels, fontsize=5)                          # y軸に卸戦略名
    plt.xlabel("小売の戦略")                                 # x軸ラベル
    plt.ylabel("卸の戦略")                                   # y軸ラベル
    plt.title(f"卸の利得ヒートマップ ({title_suffix})")       # タイトル
    plt.tight_layout()                                       # レイアウト調整

    vmin = np.min(profit_R_mat)
    vmax = np.max(profit_R_mat) * 1.2   # ← 上限を20%持ち上げる
    ...
    plt.colorbar(label="小売の利得 [円]")

    plt.xticks(range(nR), x_labels, rotation=90, fontsize=5)
    plt.yticks(range(nW), y_labels, fontsize=5)
    plt.xlabel("小売の戦略")
    plt.ylabel("卸の戦略")
    plt.title(f"小売の利得ヒートマップ ({title_suffix})")
    plt.tight_layout()
...
if __name__ == "__main__":  # このファイルが直接実行されたときだけ実行するブロック
    ...
    # MODE に応じてExcelファイル名を自動決定#######################################################ファイル名変更#####################################################################################################################
    if MODE == "S":
        excel_name = "game_results_S2.xlsx"
    elif MODE == "L":
        excel_name = "game_results_L2.xlsx"
    else:
        excel_name = "game_results_both.xlsx"  # bothモード対策（不要なら削除OK）

    plot_strategy_profit_heatmaps(
        pol_dummy,
        title_suffix="全戦略プロファイル",
    ...
    show_result("ペナルティ付き利益ゲーム", res_eq)  # 結果表示
